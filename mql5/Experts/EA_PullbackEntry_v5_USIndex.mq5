...
input double InpConfirmBarMinDollars = 1.0;  // 確認足最小サイズ(ドル)
input double InpConfirmBarMaxDollars = 0.0;  // 確認足最大サイズ(ドル) 0=無制限
input bool   InpRequirePriceBreak = false;   // 価格ブレイク待機モード
input double InpEntryBreakBufferDollars = 0.5; // エントリーバッファ(ドル)
...
input double InpStopHuntSpikePoints = 50.0;       // ストップ狩りスパイク幅(points)
input int    InpStopHuntRecoveryBars = 2;         // 回復確認バー数

//--- ラウンドナンバー (1000/500 points) 設定
input bool   InpUseRoundNumberLines = false;      // ラウンドナンバーライン使用
input bool   InpRN_Use_00_Line = true;            // 1000ライン使用
input bool   InpRN_Use_50_Line = true;            // 500ライン使用
input double InpRN_TouchBufferPoints = 20.0;      // タッチ判定バッファ(points)
input int    InpRN_LookbackBars = 3;              // 検出期間(バー数)
input bool   InpRN_CounterTrend = false;          // 逆張りモード（反転狙い）
input int    InpRN_DigitLevel = 0;                // 桁数レベル（0=整数, US Index）
input bool   InpRN_AvoidEntryNear = false;        // ラウンドナンバー付近でのエントリー回避
input double InpRN_AvoidBufferPoints = 50.0;      // 回避範囲(points)
...
input double InpATRMinDollars = 35.0;        // ATR最小値(ドル)（ドキュメント推奨: US30 35ドル）
...
int OnInit()
{
   ...
   if(InpPresetApplyMode != 2 || InpPreset == PRESET_CUSTOM)
   {
      ...
      // 確認足設定 (MT4非OOP互換) - USIndexはドル単位をポイントに変換
      cfg.UseConfirmationBar = InpUseConfirmationBar;
      cfg.ConfirmationBarMinPips = InpConfirmBarMinDollars / _Point;  // ドル→Points
      cfg.ConfirmationBarMaxPips = InpConfirmBarMaxDollars / _Point;
      cfg.RequirePriceBreak = InpRequirePriceBreak;
      cfg.EntryBreakBufferPips = InpEntryBreakBufferDollars / _Point;
      ...
      cfg.RN_AvoidBufferPoints = InpRN_AvoidBufferPoints;
