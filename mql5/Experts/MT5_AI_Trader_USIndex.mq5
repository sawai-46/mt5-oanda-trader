...
input double InpATRThresholdDollars = 5.0; // ATR最低閾値(ドル。ログは price units / MT5pt を併記)
...
{
   return "PERSIST|MT5_AIT_US|" + _Symbol + "|" + (string)g_ActiveMagicNumber + "|";
}
...
void PersistClearAllForSymbolMagic()
{
   ...
   if(InpLogPersistentTpStateEvents)
      CLogger::Log(LOG_INFO, StringFormat("[PERSIST][MT5_AIT_US] cleared GV for %s magic=%lld", _Symbol, g_ActiveMagicNumber));
}
...
void PersistSaveByTicket(const ulong ticket, const int stage)
{
   ...
   if(InpLogPersistentTpStateEvents)
      CLogger::Log(LOG_INFO, StringFormat("[PERSIST][MT5_AIT_US] saved ident=%lld ticket=%lld stage=%d", identifier, ticket, stage));
}
