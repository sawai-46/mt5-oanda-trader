...
input double InpDeviationPips = 5.0;         // 最大スリッページ(pips) ※M15推奨: 3-5pips
...
input bool   InpRequirePerfectOrder = true;  // パーフェクトオーダー必須（MT4非 OOP版準拠）
...
input double InpConfirmBarMinPips = 2.0;     // 確認足最小サイズ(pips)
input double InpConfirmBarMaxPips = 0.0;     // 確認足最大サイズ(pips) 0=無制限
input bool   InpRequirePriceBreak = false;   // 価格ブレイク待機モード
input double InpEntryBreakBufferPips = 0.5;  // エントリーバッファ(pips)
...
input double InpATRMinPips = 7.0;            // ATR最小値(pips)（ドキュメント推奨: USDJPY 7.0）

//--- SL/TP Settings
input ENUM_SLTP_MODE InpSLTPMode = SLTP_FIXED;  // SL/TPモード
input double InpSLFixedPips = 15.0;          // SL(pips) - Fixed
input double InpTPFixedPips = 30.0;          // TP(pips) - Fixed
...
input double InpPartial1Pips = 15.0;         // 1段階目(pips)
input double InpPartial1Percent = 50.0;      // 1段階目決済率(%)
input double InpPartial2Pips = 30.0;         // 2段階目(pips)
input double InpPartial2Percent = 50.0;      // 2段階目決済率(%)
input double InpPartial3Pips = 45.0;         // 3段階目(pips)
...
input double InpTrailStartPips = 20.0;       // トレーリング開始(pips)
...
{
   CLogger::Log(LOG_INFO, StringFormat("[CONFIG][PBE_FX] Symbol=%s Magic=%lld Preset=%s", _Symbol, cfg.MagicNumber, GetPresetName(preset)));
}
...
int OnInit()
{
   ...
   g_MaxSpreadPoints = (int)(InpMaxSpreadPips * g_pipMultiplier);
   g_ATRMinPoints = InpATRMinPips * g_pipMultiplier;
   g_SLFixedPoints = InpSLFixedPips * g_pipMultiplier;
   g_TPFixedPoints = InpTPFixedPips * g_pipMultiplier;
   g_Partial1Points = InpPartial1Pips * g_pipMultiplier;
   g_Partial2Points = InpPartial2Pips * g_pipMultiplier;
   g_Partial3Points = InpPartial3Pips * g_pipMultiplier;
   g_TrailStartPoints = InpTrailStartPips * g_pipMultiplier;
   g_TrailStepPoints = InpTrailStepPips * g_pipMultiplier;
   ...
   CLogger::Log(LOG_INFO, "PullbackEntry v5.12 FX (MQL5)");
   ...
   cfg.DeviationPoints = (int)(InpDeviationPips * g_pipMultiplier);
   ...
   if(InpPresetApplyMode != 2 || InpPreset == PRESET_CUSTOM)
   {
      ...
      cfg.ConfirmationBarMinPips = InpConfirmBarMinPips;
      cfg.ConfirmationBarMaxPips = InpConfirmBarMaxPips;
      cfg.RequirePriceBreak = InpRequirePriceBreak;
      cfg.EntryBreakBufferPips = InpEntryBreakBufferPips;
   ...
   posCfg.MaxSlippagePoints = (int)(InpDeviationPips * g_pipMultiplier);
