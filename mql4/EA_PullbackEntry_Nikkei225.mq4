//+------------------------------------------------------------------+
//|                                              EA_PullbackEntry_Nikkei225.mq4 |
//|                                  Pullback Entry Specialist System (Nikkei225) |
//|                          トレンド中のプルバックのみを狙う専用EA (日経225版)     |
//+------------------------------------------------------------------+
#property copyright "Pullback Entry Specialist (Nikkei225)"
#property link      ""
...
input int    BaseLotSize = 30;                   // 基準ロットサイズ（整数）
input int    MaxLotSize = 100;                   // 最大ロットサイズ（上限）
...
int OnInit()
{
   ...
   if (AutoMagicNumber) {
      ...
      g_ActiveMagicNumber = GenerateMagicNumber(EA_TYPE_PULLBACK_ENTRY_NK, Symbol(), preset_code);
...
void ExecuteEntry(bool is_long)
{
   ...
   // サービス削除済み - 調整なし
   // JP225: BaseLotSizeは整数（例: 30）
   int base_lot_jp225 = BaseLotSize;
   int adjusted_lot = base_lot_jp225;
