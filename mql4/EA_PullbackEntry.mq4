...
void CheckPartialClose()
{
   ...
   if (g_PartialCloseLevels >= 2 && partial1_executed && !partial2_executed && profit_pips >= g_PartialCloseLevel2_Pips) {
      ExecutePartialClose(2, g_PartialClosePercent2);
      return; // この Tick では1段階のみ実行し、次の Tick で次の段階を判定
   }
   
   // 第3利確（第1・第2段階が完了している場合のみ）
   if (g_PartialCloseLevels >= 3 && partial1_executed && partial2_executed && !partial3_executed && profit_pips >= g_PartialCloseLevel3_Pips) {
