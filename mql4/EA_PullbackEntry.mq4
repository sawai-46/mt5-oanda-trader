...
input int    Max_Slippage_Points = 0;            // 最大スリッページ(MT4 points) ※互換用、0=pips換算を使用
input bool   Use_Slippage_Pips_Conversion = false; // スリッページのpips→MT4 points換算を有効化（true推奨）※互換のため既定false
...
int EffectiveSlippagePoints()
{
   ...
   // 推奨: pips入力をMT4 API用MT4 pointsへ換算
   if (pip <= 0.0) {
...
void ApplyStrategyPreset()
{
   ...
   switch (Selected_Strategy) {
      case STRATEGY_STANDARD:
         ...
         Print("ADX >= 20.0、チャネル >= 20 pips、ATR >= 7 pips");
         Print("→ バランス重視、初心者推奨");
      ...
      case STRATEGY_CONSERVATIVE:
         ...
         Print("ADX >= 25.0、チャネル >= 30 pips、ATR >= 15 pips");
         Print("→ 質重視、勝率優先");
         Print("★ AI機能: inputパラメータでON/OFF可能");
         if (g_Use_Micro_Volatility_Filter) Print("  - HFTノイズ除外: ON");
         if (g_Use_Algo_Price_Levels) Print("  - アルゴ価格レベル: ON");
      ...
      case STRATEGY_V3_FILTERED:
         ...
         Print("環境フィルター: ADX>=20, チャネル>=20pips, ATR>=10pips");
