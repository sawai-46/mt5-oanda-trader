//+------------------------------------------------------------------+
//|                                       MT4_AI_Trader_v2_File.mq4 |
//|                           Phase 6 ファイルベース版 (DLL不要)     |
...
input double PartialClose1Pips = 15.0;      // 1段階目(pips) ※FX推奨値
input double PartialClose1Percent = 50.0;   // 1段階目決済率(%) ※二段階:50, 三段階:30
input double PartialClose2Pips = 30.0;      // 2段階目(pips) ※FX推奨値
input double PartialClose2Percent = 100.0;  // 2段階目決済率(%) ※二段階:100, 三段階:50
input double PartialClose3Pips = 45.0;      // 3段階目(pips) ※FX推奨値
...
//--- SL/TP設定（ポジション管理用）
input double StopLoss_Fixed_Pips = 15.0;    // 固定SL(pips) ※FX推奨値
input double TakeProfit_Fixed_Pips = 30.0;  // 固定TP(pips) ※FX推奨値
...
int OnInit()
{
   ...
   Print("=== MT4 AI Trader v4.0 File-based (FX専用) ===");
...
void AnalyzeAndTrade()
{
   ...
   {
      Print("スプレッドが広すぎます: ", DoubleToString(spread_pips, 1), " pips (閾値: ", MaxSpreadPips, " pips)");
      return;
   }
...
bool WriteRequestFile()
{
   ...
   // 終値履歴（最新→過去）。Python側はこの `prices` を反転して時系列(古い→新しい)として解析する。
   // 目安: 120本あればEMA/RSI/Antigravity(20本)に十分。
   string prices = "";
...
void ExecuteTrade(int signal, double confidence)
{
   ...
   int ticket = 0;
   int slippage = (int)MaxSlippagePoints;
...
void CheckPartialClose()
{
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      if(showDebug)
      {
         Print("[DEBUG] Ticket #", ticket, ": Level=", currentLevel, ", ProfitPips=", DoubleToString(profitPips, 1), 
               ", Target1=", PartialClose1Pips, " pips");
...
double CalculateLotSize()
{
   ...
   if(EnableLotAdjustment)
   {
      ...
      double stopLossPoints = DefaultSLPoints;
