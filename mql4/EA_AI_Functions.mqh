...
//|                                    AI対応関数群（GPU不要軽量版）  |
//+------------------------------------------------------------------+
#property copyright "AI Adaptive Trading Functions"
#property strict
...
bool CheckMicroVolatility()
{
   ...
   for (int i = 1; i <= g_Noise_Detection_Period; i++) {
      double bar_range = (iHigh(Symbol(), 0, i) - iLow(Symbol(), 0, i)) / pip;
      total_range += bar_range;
      
      if (bar_range < g_Min_Bar_Range_Pips) {
         noise_count++;
      }
   }
   ...
   if (noise_ratio > g_Noise_Ratio_Threshold) {
      LogSkipReason("HFTノイズ検出: " + DoubleToString(noise_ratio * 100, 1) + 
   ...
   if (EnableDebugLog) {
      Print("✓ マイクロボラ分析: 平均=", DoubleToString(avg_range, 2), 
            " pips, ノイズ率=", DoubleToString(noise_ratio * 100, 1), "%");
   }
...
bool CheckAlgoPriceLevels(bool is_long, double &detected_level)
{
   ...
   for (int i = 0; i < ArraySize(levels); i++) {
      ...
      double distance = MathAbs(current_price - levels[i]) / pip;
      ...
      if (distance < g_Algo_Price_Clustering) {
         ...
         if (EnableDebugLog) {
            Print("★ アルゴ価格レベル検出: ", level_name, " @ ", 
                  ...
                  " (現在価格から", DoubleToString(distance, 1), " pips)");
         }
         return true;
      }
   }
...
bool IsAlgoActiveHours()
{
   ...
   LogSkipReason("アルゴ非活発時間帯: " + IntegerToString(hour) + "時");
   return false;
}
...
void LogAILearningData(bool is_long, double entry_price, string pattern_type)
{
   ...
   if (file_handle != INVALID_HANDLE) {
      ...
      double bar_range = (iHigh(Symbol(), 0, 1) - iLow(Symbol(), 0, 1)) / pip;
      double adx_value = iADX(Symbol(), 0, 14, PRICE_CLOSE, MODE_MAIN, 1);
      double channel_width = (ema12_current - ema100_current) / pip;
      ...
      int noise_count = 0;
      for (int i = 1; i <= 10; i++) {
         double range = (iHigh(Symbol(), 0, i) - iLow(Symbol(), 0, i)) / pip;
         if (range < g_Min_Bar_Range_Pips) noise_count++;
      }
      ...
      double atr_5bars_ago = iATR(Symbol(), 0, 14, 5) / pip;
      double atr_spike_ratio = (atr_5bars_ago > 0) ? (current_atr / pip) / atr_5bars_ago : 1.0;
      ...
      FileWrite(file_handle, timestamp, Symbol(), IntegerToString(Period()), direction,
                ...
                DoubleToString(current_atr / pip, 2),
